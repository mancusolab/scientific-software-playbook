...
description: Use when reviewing scientific software architecture for boundary violations, data-duplication risks, and contract drift between ingress, pipeline, numerics, and egress layers.
...
4. Verify mathematical sanity checks are present for provided model artifacts.
5. Verify solver strategy is explicit when update rules are provided.
6. Verify translation feasibility to existing solvers (Optimistix/Lineax) is documented when applicable.
7. Find boundary leaks where raw source containers cross ingress.
8. Detect duplication paths where container and array representations coexist downstream.
9. Verify pipeline-only canonical input contracts.
10. Confirm numerics code stays array/PyTree-only and transformation-safe.
11. Verify TDD evidence exists for boundary and contract changes.
12. Verify completion claims are backed by fresh test/verification command output.
13. Report findings with severity and concrete fixes.
...
4. Verify mathematical concerns are resolved or explicitly tracked/accepted as risk.
5. If explicit update rules are present, verify solver strategy choice and translation-feasibility analysis are captured.
6. Discover layer boundaries and entrypoints.
7. Trace one full path from CLI ingress to numerics to egress.
8. Search for anti-patterns:
...
9. Check for red-green evidence:
- failing test added first for new behavior or bug fix.
- passing verification command output after changes.
10. Run readiness validator when plan exists:
- `scripts/validate-design-plan-readiness.sh <plan-path> --phase in-review` for `pre-implementation`.
- `scripts/validate-design-plan-readiness.sh <plan-path> --phase approval` for `implementation-gate`.
11. Produce the report using `docs/reviews/review-template.md`.
12. Fill weighted gates and score summary (`>= 90` required for approval recommendation).
13. Save report to `docs/reviews/YYYY-MM-DD-<slug>-architecture-review.md`.
...
6. If mathematical inconsistencies are untracked or ignored, raise at least `high` severity.
7. If explicit update rules exist and solver strategy is undocumented, raise at least `high` severity.
8. If custom solver is chosen without documented rationale against existing solvers, raise at least `medium` severity.
9. If pipeline or numerics accepts raw containers, raise `critical`.
