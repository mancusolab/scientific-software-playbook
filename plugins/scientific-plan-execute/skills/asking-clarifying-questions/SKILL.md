...
description: Use when initial design context is gathered but brainstorming has not started - resolves contradictions, disambiguates terminology, clarifies scope boundaries, and verifies assumptions to prevent building the wrong solution
...
- Asking for initial requirements before architecture kickoff
...
**Use AskUserQuestion when available** - otherwise ask directly with numbered options and the same trade-offs.
...
**Use AskUserQuestion when available** - otherwise ask directly with distinct numbered scope options.
...
Brainstorming (architecture kickoff exploration)
