...
|-------|---------------|------------|--------|
| **1. Understanding** | Ask questions (one at a time) | AskUserQuestion for choices (or direct numbered questions), agents for research | Purpose, constraints, criteria |
| **2. Exploration** | Propose 2-3 approaches | AskUserQuestion for approach selection (or direct numbered choices), agents for patterns | Architecture options with trade-offs |
...
**Don't overdo it:**
- Don't research things the model already knows well
- Don't research project-specific code (use codebase-investigator)
- Don't research for every small decision

**Balance:** Use research for external knowledge and current information. Use the model's built-in knowledge for general programming concepts.
...
- Redirect question to user via AskUserQuestion (or direct numbered question fallback)
...
2. **Then gather requirements:**
   ...
   - **Use AskUserQuestion tool** when available; otherwise ask directly with numbered options
...
**If architecture kickoff already gathered context:**
...
2. **Then propose approaches:**
   ...
   - **Use AskUserQuestion tool** to present approaches as structured choices; if unavailable, ask directly with numbered options
...
If AskUserQuestion is unavailable in your runtime, ask the same question directly and present 2-4 concise numbered options with the same trade-offs.
...
|-----------|-------------|
| **One question at a time** | YOU MUST ask single questions in Phase 1, use AskUserQuestion for choices (or direct numbered fallback) |
...
| **Structured choices** | YOU MUST use AskUserQuestion for 2-4 options with trade-offs, or ask directly with numbered options when unavailable |
...
| **Task tracking** | YOU MUST create task todos at start with TaskCreate, update with TaskUpdate as you progress (or `TodoWrite` in runtimes that still expose it) |
...
**Next step:** The orchestrating architecture workflow will invoke writing-design-plans to document this design.
