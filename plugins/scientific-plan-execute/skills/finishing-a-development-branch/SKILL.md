...
Present exactly these 4 options via `AskUserQuestion` when available; otherwise ask directly using the same numbered options.
...
Before merging or creating a PR, use the `maintaining-project-context` skill to update AGENTS.md/CLAUDE.md files if contracts or structure changed.
...
**If context updates were committed:** include those commits in the merge/PR.
**If no updates were needed:** proceed with the chosen option.
**If the skill is unavailable:** perform the same checks directly.
