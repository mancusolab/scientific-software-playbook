...
Acceptance Criteria translate the Definition of Done into specific, verifiable items that become the basis for test requirements during implementation. You have full context from writing the phases—do this inline; no delegation required.
...
Present generated criteria to the user. Use AskUserQuestion when available; otherwise ask directly with the same choices:
"Review the acceptance criteria. Approve to continue, or describe what's missing or needs revision."
...
After appending the body (Architecture through Additional Considerations), generate Summary and Glossary using a delegate agent with fresh context (or directly if delegation is unavailable).

**Why use delegation here?**
- Fresh context avoids "context rot" from the long brainstorming/writing session
- Acts as a forcing function: if a delegate can't extract a coherent summary, the document is unclear
...
"Glossary includes [X terms]. Omitted as likely obvious: [list from extraction pass]. Let me know if any should be included."

Don't wait for approval — proceed to insert the sections. The user can hit escape and ask for adjustments if needed.

**Step 4: Replace placeholders**

Replace the Summary and Glossary placeholder comments with the extracted output. Do not insert the Omitted Terms section — that was for your transparency message only.

**Step 5: Review and adjust**

Briefly review the generated sections for accuracy. The extraction pass may miss nuance from the conversation — adjust if needed, but prefer the extracted version when accurate (it reflects the document itself, not memory).
...
Writing Design Plans (this skill)
  ...
  -> Run extraction pass to generate Summary and Glossary
