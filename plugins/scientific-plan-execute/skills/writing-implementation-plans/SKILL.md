...
- Instruct that pass to seek out CLAUDE.md or AGENTS.md files that include testing behavior, logic, and methodology, and include file references for your plan.

Run a second codebase-investigator pass (simultaneously when delegation is available) with:
...
Before creating the Finalization task, check if `.scientific/implementation-plan-guidance.md` exists. If it does, include its absolute path in the task description:

```markdown
# If .scientific/implementation-plan-guidance.md exists:
- [ ] Finalization: Run code-reviewer over all phase files (guidance: [absolute path to .scientific/implementation-plan-guidance.md]), fix ALL issues including minor ones
      → blocked by: all Phase *D tasks

# If .scientific/implementation-plan-guidance.md does NOT exist:
...
Use `update_plan` updates (or `TaskUpdate` where available) to mark each sub-task as in_progress when starting, completed when done.
...
6. **Use AskUserQuestion when available (otherwise ask directly with the same options):**
...
- [ ] Run an Opus-level generation pass for test requirements from Acceptance Criteria
