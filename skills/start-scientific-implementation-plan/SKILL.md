...
- `docs/implementation-plans/YYYY-MM-DD-<slug>/`
6. Create/update files using templates:
- `docs/implementation-plans/templates/scientific-implementation-plan-template.md`
- `docs/implementation-plans/templates/test-requirements-template.md`
- `docs/implementation-plans/templates/phase-template.md`
- `docs/implementation-plans/templates/phase-simulation-validation-template.md` (when simulation scope is `yes`)
7. Define phased plan:
...
8. Run `scientific-internet-research-pass` when implementation tasks rely on uncertain external facts.
 - Record citations and decision impacts in implementation-plan `README.md`.
9. Build AC-to-task and AC-to-test traceability:
...
10. Add phase-level verification commands and reviewer checkpoints.
11. Set handoff decision:
- `ready-to-execute` only if all AC rows have task IDs and test IDs
- otherwise `blocked` with explicit blockers.
12. Resolve and verify absolute implementation plan directory path.
13. If handoff is `ready-to-execute`, emit a copy-paste execution command using the absolute path and recommend running it in a fresh session/context.
